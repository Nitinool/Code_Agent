...
AGENT_MAX_TOKENS=8192                 # 可选，长文档可调到 16384
AGENT_TEMPERATURE=0.7                 # 可选
AGENT_PERMISSION=normal               # normal | accept-all
```

> 💡 **关于 `AGENT_MAX_TOKENS`**：这是**单次回复**的输出上限，不是上下文窗口。
> 默认 **8192**（之前的 4096 对中文长回复经常不够，会被静默截断）。
> 长文档生成 / 大段代码场景建议调到 `16384`。中文 1 字 ≈ 1.5–2 token。
...
```
[⚠ Output truncated at max_tokens=8192. Try a larger AGENT_MAX_TOKENS, or ask me to continue.]
```
